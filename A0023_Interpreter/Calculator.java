...
class Context {
  ...
  public int getValue(String var) {
    final Integer objInt = (Integer) varList.get(var);
...
public class Calculator {
  ...
  public int evaluate() {
    //infix to Postfix
    final String pfExpr = infixToPostFix(expression);

    //build the Binary Tree
    final Expression rootNode = buildTree(pfExpr);
  ...
  private Expression buildTree(String expr) {
    ...
    for (int i = 0; i < expr.length(); i++) {
      final String currChar = expr.substring(i, i + 1);

      if (isOperator(currChar) == false) {
        final Expression e = new TerminalExpression(currChar);
        s.push(e);
      } else {
        final Expression r = (Expression) s.pop();
        final Expression l = (Expression) s.pop();
        final Expression n = getNonTerminalExpression(currChar, l, r);
        s.push(n);
      }
    }//for
  ...
  private String infixToPostFix(String str) {
    ...
    final String expr = str.trim();
    for (int i = 0; i < str.length(); i++) {

      final String currChar = str.substring(i, i + 1);
      ...
      if (isOperator(currChar)) {
        if (s.isEmpty() == false) {
          ...
          final String strVal2 = (String) operators.get(currChar);
          final int val2 = new Integer(strVal2).intValue();
